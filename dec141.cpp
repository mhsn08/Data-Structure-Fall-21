...
public:
    ...
    node * find_parent(int , node *);
    void brother_node(int);
    node * find_brother(int , node *);
};
...
int main()
{
    ...
    cout << endl;
    // tree.parent_node(55);
    // tree.parent_node(119);
    // tree.parent_node(78);
    tree.brother_node(55);
    tree.brother_node(67);
    tree.brother_node(119);
    return 0;
}
...
void BST::parent_node(int key)
{
    if(root != NULL)
    {
        ...
        node * temp = find_parent(key , root);
        if(temp != NULL)
            cout << key << " 's Parent = " << temp->info << endl;
        else
...
node * BST::find_parent(int key ,node * temp)
{
    if (temp == NULL)
        return temp;
    if(temp->left != NULL)
    ...
    if(temp->info > key)
        return find_parent(key , temp->left);
    else
        return find_parent(key , temp->right);
}


void BST::brother_node(int key)
{
    if(root != NULL)
    {
        node * temp = find_brother(key , root);
        if(temp != NULL)
            cout << temp->info << endl;
        else
...
node * BST::find_brother(int key , node * temp)
{
    if(temp == NULL)
        return temp;
    if(temp->info == key)
    ...
    if(temp->info > key)
        return find_brother(key , temp->left);
    else
        return find_brother(key , temp->right);
}
